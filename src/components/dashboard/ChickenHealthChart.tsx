...
const CustomTooltip = ({ active, payload, theme }: any) => {
  if (active && payload && payload.length) {
    return (
      <div
        className={`${theme === "dark" ? "bg-slate-800" : "bg-white"} border border-slate-200 dark:border-slate-700 p-3 rounded-lg shadow-xl backdrop-blur-md`}
      >
        <p
          className={`${theme === "dark" ? "text-slate-300" : "text-slate-600"} font-medium text-sm`}
        >
          {payload[0].name}
        </p>
        <p
          className={`${theme === "dark" ? "text-white" : "text-slate-900"} font-bold text-lg`}
        >
          {payload[0].value}{" "}
          <span
            className={`${theme === "dark" ? "text-slate-400" : "text-slate-500"} text-xs font-normal`}
          >
            scans
          </span>
        </p>
...
export default function ChickenHealthChart() {
  ...
  // 👈 CALL THE DATA HOOK
  const { data, isLoading, error, totalScans, refreshData } =
  ...
  if (error) {
    return (
      <div
        className={`${baseClassName} ${cardBg} justify-center items-center text-center p-8 bg-red-50 dark:bg-red-900/40 border-red-300 dark:border-red-700`}
      >
    ...
    return (
      <div className={`${baseClassName} ${cardBg} justify-center items-center`}>
        <p className="font-medium text-slate-500 dark:text-slate-300">
          No scan data found for this user/farm.
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      className={`w-full backdrop-blur-xl border rounded-2xl p-6 ${cardBg} transition-colors duration-300`}
    >
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-2">
        <div>
          <h2
            className={`text-xl md:text-2xl font-bold ${theme === "dark" ? "text-white" : "text-slate-800"} transition-colors`}
          >
            Scan Overview
          </h2>
          <p
            className={`text-sm ${theme === "dark" ? "text-slate-400" : "text-slate-500"} mt-1`}
          >
        ...
        <div className="flex gap-3 items-center">
          ...
          >
            ...
            >
              <path d="M21 12a9 9 0 0 0-9-9c-2.4 0-4.64.65-6.53 1.77L3 7" />
              <path d="M3 3v4h4" />
              <path d="M3 12a9 9 0 0 0 9 9c2.4 0 4.64-.65 6.53-1.77L21 17" />
              <path d="M21 21v-4h-4" />
            </svg>
          </button>

          <div
            className={`border ${theme === "dark" ? "bg-green-500/10 text-green-400 border-green-500/20" : "bg-green-100 text-green-700 border-green-200"} px-3 py-1 rounded-full text-xs font-medium`}
          >
            Live Data
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="h-[300px] w-full md:w-1/2 relative">
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
            <span
              className={`block text-3xl font-extrabold ${theme === "dark" ? "text-white" : "text-slate-800"} transition-colors`}
            >
              {totalScans}
            </span>
            <span
              className={`block text-xs ${theme === "dark" ? "text-slate-400" : "text-slate-500"} uppercase tracking-wider`}
            >
              Total
            </span>
          </div>
        ...
        <div className="w-full md:w-1/2 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
          <h3
            className={`text-xs font-semibold ${theme === "dark" ? "text-slate-500" : "text-slate-400"} uppercase tracking-widest mb-3`}
          >
            Breakdown
          </h3>
              ...
              >
                <div className="flex items-center gap-3">
                  <span
                    ...
                    style={{
                      backgroundColor: labelColors[item.name] || "#ccc",
                    }}
                  />
                  <span
                    className={`text-sm font-medium ${theme === "dark" ? "text-slate-300 group-hover:text-white" : "text-slate-600 group-hover:text-slate-900"} transition-colors`}
                  >
                    {item.name}
                  </span>
                </div>
