...
const StatCard: React.FC<StatCardProps> = ({
  ...
  return (
    <div
      className={`backdrop-blur-xl border ${cardBg} rounded-2xl p-6 shadow-sm dark:shadow-xl transition-colors h-full flex flex-col justify-between`}
    >
      <div className="flex items-start justify-between">
        <div>
          <h3
            className={`${theme === "dark" ? "text-slate-400" : "text-slate-500"} text-xs uppercase font-bold tracking-wider`}
          >
            {title}
          </h3>
          <div className={`text-3xl md:text-4xl font-bold mt-2 ${colorClass}`}>
            {value}
          </div>
        </div>
      ...
      {showProgressBar && (
        <div
          className={`mt-4 w-full ${theme === "dark" ? "bg-slate-700" : "bg-slate-100"} h-2 rounded-full overflow-hidden`}
        >
