...
export const VideoUpload: React.FC = () => {
  ...
  // Use the socket hook
  const { result, startDetection, stopDetection } =
  ...
  const handleDetectionResult = useCallback(
    async (detection: DetectionResult) => {
      ...
      if (DISEASES_TO_SAVE.has(diagnosis)) {
        console.log(
          `Saving diagnosis: ${diagnosis} with confidence ${detection.confidence}`,
        );
        ...
        if (success) {
          ...
          console.error(
            `FAILED to save scan for ${diagnosis}. Check useScanInsertion hook for API error details.`,
          );
        }
      }
    },
